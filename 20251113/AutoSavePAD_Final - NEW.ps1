...
function Try-SendSave {
    ...
    if (-not (Is-PADRunning)) { Write-Log "Try-SendSave: PAD not running"; return $false }
    ...
    while ($attempt -lt $maxAttempts) {
        ...
        if (-not $pad) { Write-Log "No PAD process selected or found; aborting attempt"; return $false }

        try { Write-Log ("Selected PAD: Id={0}; Title=`"{1}`"; Handle={2}; Responding={3}" -f $pad.Id, $pad.MainWindowTitle, $pad.MainWindowHandle, $pad.Responding) } catch {}
        ...
        if (-not $focused) {
            try {
                ...
                if (-not $ok -and $pad.MainWindowTitle) { try { $ok = $wsh.AppActivate($pad.MainWindowTitle) } catch {} }
        ...
        if (-not $focused) {
            Write-Log "Could not focus selected PAD; will retry after backoff"
        ...
        # Tạo form tạm để đẩy hộp thoại lên trên cùng
		...
		$formTop.Show()
		$res = [System.Windows.Forms.MessageBox]::Show($formTop, $message, $caption, [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question, [System.Windows.Forms.MessageBoxDefaultButton]::Button1)
		$formTop.Dispose()
		...
		# Ghi log và thông báo kèm tiêu đề cửa sổ
        ...
        if (-not $sent) {
            try { [System.Windows.Forms.SendKeys]::SendWait('^s'); Start-Sleep -Milliseconds 400; $sent = $true; Write-Log "Sent Ctrl+S via SendKeys.SendWait" } catch { Write-Log ("SendKeys.SendWait failed: {0}" -f $_.Exception.Message) }
        }
        ...
        if ($sent) {
            ...
            Write-Log "Send failed; will retry"
        }
