# AutoSavePAD_Final_FullFixed_v4_fixed.ps1
# Auto-Save Power Automate Desktop (PAD) — tray app
# Fixes: full functions included; safe form registration; call Show-SettingsWindow with &
...
# PAD helpers
function Get-AllPADProcesses {
    try {
        Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 -and $_.MainWindowTitle -and ($_.MainWindowTitle -like "*Power Automate*") } | Sort-Object Id
    } catch { @() }
}
function Get-PADMainProcess { Get-AllPADProcesses | Select-Object -First 1 }
function Is-PADRunning { return ((Get-AllPADProcesses).Count -gt 0) }
function Is-PADResponsive { $p = Get-PADMainProcess; if (-not $p) { return $false }; return $p.Responding }

# Window chooser UI
function Choose-PADWindow {
    param([array]$procs)
    if (-not $procs -or $procs.Count -eq 0) { return $null }
    if ($procs.Count -eq 1) { return $procs[0] }

    $form = New-Object System.Windows.Forms.Form
    $form.Text = "Chọn cửa sổ Power Automate để lưu"
    $form.Size = New-Object System.Drawing.Size(640,380)
    $form.StartPosition = "CenterScreen"
    $form.TopMost = $true

    $lbl = New-Object System.Windows.Forms.Label
    $lbl.Text = "Tìm thấy nhiều cửa sổ. Chọn một cửa sổ để gửi lệnh Lưu (Ctrl+S):"
    $lbl.AutoSize = $true
    ...
    $list.Size = New-Object System.Drawing.Size(600,260)
    ...
    $col3.Text = "Responding"
    $col3.Width = 80

    $list.Columns.AddRange(@($col1,$col2,$col3))
    $form.Controls.Add($list)

    foreach ($p in $procs) {
        $item = New-Object System.Windows.Forms.ListViewItem($p.Id.ToString())
    ...
    $btnOk.Location = New-Object System.Drawing.Point(360,305)
    $btnOk.Add_Click({
        if ($list.SelectedItems.Count -gt 0) { $form.Tag = $list.SelectedItems[0].Tag; $form.Close() } else { [System.Windows.Forms.MessageBox]::Show("Vui lòng chọn một cửa sổ.","Chọn", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) }
    })
    ...
    $btnCancel.Location = New-Object System.Drawing.Point(492,305)
    ...
    if ($form.Tag) { return $form.Tag } else { return $null }
}
...
function Safe-BringPADToFront {
    ...
    try {
        ...
        try {
            ...
            if ($isIconic) { [WinAPI_User32]::ShowWindow($hWnd,9) | Out-Null; Start-Sleep -Milliseconds 200 }
        } catch {}
        try { if ([WinAPI_User32]::SetForegroundWindow($hWnd)) { return $true } } catch {}
        try { [WinAPI_User32]::BringWindowToTop($hWnd) | Out-Null; Start-Sleep -Milliseconds 120 } catch {}
        try {
            $fg = [WinAPI_User32]::GetForegroundWindow()
            $curThread = [WinAPI_User32]::GetCurrentThreadId()
            $out1 = 0
            ...
            if ($targetThread -ne 0 -and $winThread -ne 0) {
                if ([WinAPI_User32]::AttachThreadInput($curThread,$targetThread,$true)) {
                    Start-Sleep -Milliseconds 80
                    [WinAPI_User32]::SetForegroundWindow($hWnd) | Out-Null
                    [WinAPI_User32]::AttachThreadInput($curThread,$targetThread,$false) | Out-Null
                    Start-Sleep -Milliseconds 80
                    if ([WinAPI_User32]::GetForegroundWindow() -eq $hWnd) { return $true }
                }
            }
        } catch {}
        return $false
...
function Is-FlowRunning {
    try {
        $hostProc = Get-Process -Name "PAD.Console.Host" -ErrorAction SilentlyContinue
        if ($hostProc -and $hostProc.Count -gt 0) {
            Write-Log "PAD.Console.Host is running — flow or debug in progress"
            return $true
        }
        return $false
    } catch {
        Write-CriticalLog ("Is-FlowRunning error: {0}" -f $_.Exception.Message)
        return $false
    }
}

# Try-SendSave (with chooser)
function Select-PADProcess {
    $procs = Get-AllPADProcesses
    if (-not $procs -or $procs.Count -eq 0) { return $null }
    if ($procs.Count -eq 1) { return $procs[0] }
    return Choose-PADWindow -procs $procs
}

function Try-SendSave {
    param(
        [int]$maxAttempts = $script:Settings.MaxRetries,
        [int]$initialBackoff = $script:Settings.InitialBackoffMs,
        [int]$maxBackoff = $script:Settings.MaxBackoffMs
    )

    Write-Log ("Try-SendSave started (maxAttempts={0})" -f $maxAttempts)

    if (-not (Is-PADRunning)) {
        Write-Log "Try-SendSave: PAD not running"
        return $false
    }

    if (-not (Is-PADResponsive)) {
        Write-Log "Try-SendSave: PAD not responding"
        return $false
    }

    if (-not (Is-InSchedule $script:Settings.ScheduleStart $script:Settings.ScheduleEnd)) {
        Write-Log "Try-SendSave: Outside schedule"
        return $false
    }

    if (Is-FlowRunning) {
        Write-Log "Try-SendSave: Flow running; skip"
        Show-Toast "Đang chạy flow, không thực hiện lưu"
        return $false
    }

    $pad = Select-PADProcess
    if (-not $pad) {
        Write-Log "Người dùng đã hủy chọn cửa sổ hoặc không tìm thấy cửa sổ phù hợp. Bỏ qua lưu lần này."
        Show-Toast "Đã hủy chọn cửa sổ — không lưu"
        return $false
    }

    try {
        Write-Log ("Selected PAD: Id={0}; Title=`"{1}`"; Handle={2}; Responding={3}" -f $pad.Id, $pad.MainWindowTitle, $pad.MainWindowHandle, $pad.Responding)
    } catch {}
    ...
    if (-not $focused) {
        try {
            ...
            $ok = $false
            try { $ok = $wsh.AppActivate($pad.Id) } catch {}
            if (-not $ok -and $pad.MainWindowTitle) {
                try { $ok = $wsh.AppActivate($pad.MainWindowTitle) } catch {}
            }
    ...
    if (-not $focused) {
        Write-Log "Could not focus selected PAD; skipping save"
        return $false
    }

    # Show confirmation popup on top
    $caption = "Auto-Save PAD"
    $message = "Auto-Save dự định gửi lệnh Lưu (Ctrl+S) cho cửa sổ đã chọn:`n`n{0}`n`nBạn có muốn tiếp tục?" -f $pad.MainWindowTitle
    $formTop = New-Object System.Windows.Forms.Form
    $formTop.TopMost = $true
    $formTop.StartPosition = "CenterScreen"
    $formTop.ShowInTaskbar = $false
    $formTop.Size = New-Object System.Drawing.Size(0,0)
    $formTop.Show()
    $res = [System.Windows.Forms.MessageBox]::Show($formTop, $message, $caption,
    ...
    if ($res -ne [System.Windows.Forms.DialogResult]::Yes) {
        Write-Log "User chose NOT to save this cycle."
        Show-Toast "Bỏ qua lưu lần này"
        return $false
    }

    Write-Log ("Đã chọn cửa sổ để lưu: {0}" -f $pad.MainWindowTitle)
    Show-Toast ("Đang lưu flow: {0}" -f $pad.MainWindowTitle)
    ...
    try {
        ...
        Write-Log ("Wsh.SendKeys failed: {0}" -f $_.Exception.Message)
    }
    ...
    if (-not $sent) {
        try {
            ...
            Write-Log ("SendKeys.SendWait failed: {0}" -f $_.Exception.Message)
        }
    }
    ...
    if ($sent) {
        ...
        Update-Tooltip
        Save-State
        Show-Toast ("Đã lưu lúc {0}" -f (Get-Date -Format 'HH:mm:ss'))
        return $true
    } else {
        Write-Log "Send attempt failed"
...
# Show-SettingsWindow (defined before assigning menu handler)
...
# Menu handlers (assign AFTER functions defined)
...
# Important: invoke with & so function is resolved in current scope
...
# Ensure and register Form events safely
try {
    if ($script:Form -and ($script:Form -is [System.Windows.Forms.Form])) {
        if (-not $script:__FormHandlersRegistered) {
            ...
            $script:Form.Load.Add({
                try {
                    ...
                    Update-Tooltip
                    try { if ($script:IdleWatcher) { $script:IdleWatcher.Start() } } catch { Write-Log ("IdleWatcher start failed: {0}" -f $_.Exception.Message) }
                    Write-Log "AutoSavePAD started"
                } catch { Write-CriticalLog ("Form load error: {0}" -f $_.Exception.Message) }
            })
            $script:__FormHandlersRegistered = $true
            Write-Log "Form handlers registered"
        } else { Write-Log "Form handlers already registered; skipping" }
    } else {
        Write-Log "Warning: form not created; skipping form event registration"
    }
} catch { Write-CriticalLog ("Registering form handlers failed: {0}" -f $_.Exception.Message) }

# Restore state and start timers
Load-State
# Khởi động Auto-Save nếu trạng thái đã lưu là ON
if ($script:AutoSaveEnabled) {
    Write-Log "Auto-Save được bật từ trạng thái đã lưu"
    if ($script:MiStatus) { $script:MiStatus.Text = "Auto-Save: ON" }
    if ($script:MiStartStop) { $script:MiStartStop.Text = "Tắt Auto-Save" }
    try {
        $script:Timer.Interval = $script:Settings.IntervalMs
        ...
        Write-Log ("Failed to start Timer on restore: {0}" -f $_.Exception.Message)
    }
    Update-Tooltip
    Save-State
} else {
    Write-Log "Auto-Save đang tắt theo trạng thái đã lưu"
    if ($script:MiStatus) { $script:MiStatus.Text = "Auto-Save: OFF" }
    if ($script:MiStartStop) { $script:MiStartStop.Text = "Bật Auto-Save" }
    Update-Tooltip
}
...
try {
    if ($script:AutoSaveEnabled -and -not $script:InSleepMode) {
        $script:Timer.Interval = $script:Settings.IntervalMs
        try { $script:Timer.Start() } catch { Write-Log ("Timer start failed: {0}" -f $_.Exception.Message) }
        Write-Log "Auto-Save resumed"
    }
} catch { Write-CriticalLog ("Restore/start failed: {0}" -f $_.Exception.Message) }

# Message loop or fallback
try {
    if ($script:Form -and ($script:Form -is [System.Windows.Forms.Form])) {
