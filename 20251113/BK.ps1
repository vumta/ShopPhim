# AutoSavePAD_Final_Complete.ps1
# Auto-Save Power Automate Desktop (PAD) — tray app (complete, null-safe, STA, Show-SettingsWindow integrated)
# Save as UTF-8 with BOM. Run with:
#   powershell.exe -STA -ExecutionPolicy Bypass -File "C:\Scripts\AutoSavePAD_Final_Complete.ps1"

# Relaunch as STA if not already
        ...
        try {
            if (Get-Module -Name BurntToast -ListAvailable) {
                try {
                    ...
                    New-BurntToastNotification -Text "Auto-Save PAD", $message -ErrorAction Stop
                    return
                } catch { Write-Log ("BurntToast invocation failed: {0}" -f $_.Exception.Message) }
            }
        } catch {}
        ...
        try {
            if ($script:NotifyIcon) {
                try {
                    ...
                    return
                } catch { Write-Log ("NotifyIcon balloon failed: {0}" -f $_.Exception.Message) }
            }
            ...
            try { Write-Log ("Show-Toast final fallback error: {0}" -f $_.Exception.Message) } catch {}
        }
    }
}

# ---------------- Trap to log exceptions with context ----------------
trap {
    try {
        $err = $_.Exception
...
# ---------------- Safe Add-Type helper and embedded helpers ----------------
function SafeAddType { param([string]$TypeName,[string]$SourceCode)
    try { if ([type]::GetType($TypeName,$false,$false)) { Write-Log ("SafeAddType: {0} exists" -f $TypeName); return $true } } catch {}
    try { Add-Type -TypeDefinition $SourceCode -ErrorAction Stop; Write-Log ("SafeAddType: added {0}" -f $TypeName); return $true } catch {
...
SafeAddType -TypeName "IdleTimeHelper" -SourceCode $idleCode | Out-Null
SafeAddType -TypeName "WinAPI_User32" -SourceCode $winApiCode | Out-Null

function Get-IdleTime {
    try {
        if ([type]::GetType("IdleTimeHelper",$false,$false)) { return [IdleTimeHelper]::GetIdleTime() }
        Write-CriticalLog "Get-IdleTime: IdleTimeHelper missing"
        return [TimeSpan]::Zero
    } catch { Write-CriticalLog ("Get-IdleTime error: {0}" -f $_.Exception.Message); return [TimeSpan]::Zero }
}
...
function Safe-BringPADToFront { param($proc) try { if (-not $proc) { return $false }; if ([type]::GetType("WinAPI_User32",$false,$false)) { [WinAPI_User32]::SetForegroundWindow($proc.MainWindowHandle) | Out-Null; Start-Sleep -Milliseconds 450; return $true }; return $false } catch { Write-Log ("Safe-BringPADToFront error: {0}" -f $_.Exception.Message); return $false } }
...
# ---------------- Show-SettingsWindow (script-scoped, safe) ----------------
...
function Try-SendSave {
    ...
    if (-not (Can-SendKeysToPAD)) { Write-Log "Try-SendSave: Cannot send keys to PAD"; return $false }
    ...
    while ($attempt -lt $maxAttempts) {
        ...
        try { $fg = [WinAPI_User32]::GetForegroundWindow(); Write-Log ("Foreground handle: {0}" -f $fg) } catch {}
        ...
        if ($focused) {
            ...
            foreach ($v in $variants) {
                try {
                    Write-Log ("Sending variant: {0}" -f $v)
                    [System.Windows.Forms.SendKeys]::SendWait($v)
                    Start-Sleep -Milliseconds 800
                    $sentOk = $true
                    Write-Log ("SendWait succeeded for {0}" -f $v)
                    break
                } catch { Write-Log ("SendWait {0} error: {1}" -f $v, $_.Exception.Message) }
            }
            if ($sentOk) {
                Start-Sleep -Milliseconds 800
                $script:ConsecutiveFailures = 0
        ...
        } else { Write-Log "Could not focus PAD" }
...
$script:Timer.Add_Tick({
    try {
        ...
        if ($idle.TotalMilliseconds -ge $script:IdleLimitMs) {
            ...
            $script:InSleepMode = $true; $script:AutoSaveEnabled = $false
...
$script:MiSettings.Add_Click({
    try { Show-SettingsWindow } catch { Write-CriticalLog ("MiSettings handler error: {0}" -f $_.Exception.Message) }
})
...
# ---------------- Restore state and run message loop ----------------
Load-State
...
# Hidden form to run message loop
$script:Form = New-Object System.Windows.Forms.Form
$script:Form.Size = New-Object System.Drawing.Size(0,0)
$script:Form.ShowInTaskbar = $false
$script:Form.WindowState = "Minimized"
$script:Form.Add_Shown({ try { $script:Form.Hide() } catch {} })
$script:Form.Load.Add({
    try {
        $script:Form.Hide()
        Update-Tooltip
        try { $script:IdleWatcher.Start() } catch {}
        Write-Log "AutoSavePAD started"
    } catch { Write-CriticalLog ("Form load error: {0}" -f $_.Exception.Message) }
})

try { [System.Windows.Forms.Application]::Run($script:Form) } catch { Write-CriticalLog ("Application loop error: {0}`n{1}" -f $_.Exception.Message, $_.Exception.StackTrace) }